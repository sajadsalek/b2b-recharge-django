...
@pytest.mark.django_db(transaction=True)
def test_external_charge_request_with_threading(user, user1):
    ...
    # اعتبار نهایی باید با جمع تراکنش‌ها برابر باشد
    ...
    #
    #     _sum = sum(tr.amount for tr in transaction)
    #     assert wal1.remaining == _sum
    #
    #
    # for _ in range(10):
    #     create_refill_and_approved(user=user)
    ...
    #
    # transactions = transaction_list(user=user)
    # transactions1 = transaction_list(user=user1)
    # assert 10 == len(transactions)
    # assert 10 == len(transactions1)
    #
    # AMOUNT = Decimal("1.00")
    # results = []
    #
    # def withdraw(u: CustomUser):
    #     try:
    #         external_charge_request_(amount=AMOUNT, phone_number="09122222222", user=u)
    #         results.append(True)
    #
    #     finally:
    #         results.append(False)
    #
    # # t1 = threading.Thread(target=withdraw, args=(user,))
    # # t2 = threading.Thread(target=withdraw, args=(user1,))
    # # t3= threading.Thread(target=withdraw, args=(user,))
    # # t4 = threading.Thread(target=withdraw, args=(user1,))
    # #
    # # t1.start()
    # # t2.start()
    # # t3.start()
